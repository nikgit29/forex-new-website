...
import MOB_OPEN_LOCK_RATE_MODAL from "../../../Redux/Action/MobAction/mobopenlockrate.action";
...
const MobAlertHead = () => {
  ...
  const Shows = response && response.MOB_OPEN_CLOSE_LOCK_RATE.Box;
  const personId =
    response &&
  ...
  const openLockRate = (rate_alert, currencyName, filledAmount, radio) => {
    ...
    dispatch(MOB_OPEN_LOCK_RATE_MODAL());
  ...
  const getSubmitData = async (e) => {
    ...
    if (Index == 3 && amount >= checkLiveRate) {
      ...
      dispatch(MOB_CLOSE_LOCK_RATE_MODAL());
