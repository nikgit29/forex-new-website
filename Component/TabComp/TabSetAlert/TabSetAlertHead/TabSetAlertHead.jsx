...
import TAB_OPEN_LOCK_RATE_MODAL from "../../../Redux/Action/MobAction/tabopenlockrate.action";
import TAB_CLOSE_LOCK_RATE_MODAL from "../../../Redux/Action/MobAction/tabcloselockrate.action";
...
const TabAlertHead = () => {
  ...
  const Shows = response && response.TAB_OPEN_CLOSE_LOCK_RATE.Box;
  const personId =
    response &&
  ...
  const openLockRate = (rate_alert, currencyName, filledAmount, radio) => {
    ...
    dispatch(TAB_OPEN_LOCK_RATE_MODAL());
  ...
  const getSubmitData = async (e) => {
    ...
    if (Index == 3 && amount >= checkLiveRate) {
      ...
      dispatch(TAB_CLOSE_LOCK_RATE_MODAL());
