...
const TechnicalCharts = () => {
  ...
  const GetBeforeLoginImage = async () => {
    try {
      ...
      setCurrencyImage(response && response.data[2] && response.data[2].image);
      setSilverImage(response && response.data[1] && response.data[1].image);
      setGoldImage(response && response.data[0] && response.data[0].image);
    } catch (err) {
      console.log(err);
    }
  };
  ...
  const Design = (
    <>
      ...
      {/* Start Modal open when client is unpaid plan */}
      <Modal
        show={open}
      ...
      {/* End  Modal open when client is unpaid plan*/}
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title id="example-custom-modal-styling-title">
            <span style={{ marginLeft: "400px" }}>TECHNICAL CHART</span>
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
